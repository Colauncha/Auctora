...
def get_wallet_repo(db: AsyncSession = Depends(get_async_db)):
...
def get_payment_repo(db: AsyncSession = Depends(get_async_db)):
    PaymentRepo = factory.payment_repo
    return PaymentRepo(db)


def get_chat_repo(db: AsyncSession = Depends(get_async_db)):
    ChatRepo = factory.chat_repo
    return ChatRepo(db)


def get_blog_repo(db: AsyncSession = Depends(get_async_db)):
    BlogRepo = factory.blog_repo
    return BlogRepo(db)


def get_blog_comment_repo(db: AsyncSession = Depends(get_async_db)):
    BlogCommentRepo = factory.blog_comment_repo
    return BlogCommentRepo(db)


def get_auction_p_repo(db: AsyncSession = Depends(get_async_db)):
...
def get_bid_repo(db: AsyncSession = Depends(get_async_db)):
    BidRepo = factory.bid_repo
    return BidRepo(db)


def get_category_repo(db: AsyncSession = Depends(get_async_db)):
    CategoryRepo = factory.category_repo
    return CategoryRepo(db)


def get_sub_category_repo(db: AsyncSession = Depends(get_async_db)):
    SubCategoryRepo = factory.sub_category_repo
    return SubCategoryRepo(db)


def get_item_repo(db: AsyncSession = Depends(get_async_db)):
    ItemRepo = factory.item_repo
    return ItemRepo(db)


def get_notification_repo(db: AsyncSession = Depends(get_async_db)):
    NotifRepo = factory.notif_repo
    return NotifRepo(db)


def get_rewardhistory_repo(db: AsyncSession = Depends(get_async_db)):
